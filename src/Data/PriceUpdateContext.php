...
class PriceUpdateContext
{
    ...
    {
		Log::channel('price')->notice('Price Update - Token valid? = ' . (time() - (int)(config('shopware-api.access_token_expires_at.price_update'))));
		...
		if (time() - (int)(config('shopware-api.access_token_expires_at.price_update')) > (8 * 60)) {
			...
			config([
				'shopware-api.access_token.price_update' => $accessToken,
				'shopware-api.access_token_expires_at.price_update' => strtotime('now'),
			]);
