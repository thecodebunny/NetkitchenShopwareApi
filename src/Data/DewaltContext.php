...
class DewaltContext
{
    ...
    {
		Log::channel('stock')->notice('dewalt Token valid? = ' . (time() - (int)(config('shopware-api.access_token_expires_at.dewalt'))));

		if (time() - (int)(config('shopware-api.access_token_expires_at.dewalt')) > (8 * 60)) {
			Log::channel('stock')->notice('Getting new token.');
			//Artisan::call('config:clear');
			$grantType = new ClientCredentialsGrantType(config('shopware-api.access_key.dewalt'),config('shopware-api.secret_access_key.dewalt'));
			...
			config([
				'shopware-api.access_token.dewalt' => $accessToken,
				'shopware-api.access_token_expires_at.dewalt' => strtotime('now'),
			]);
			...
			$accessToken = config('shopware-api.access_token.dewalt');
		}
		return $accessToken;
    }
}
