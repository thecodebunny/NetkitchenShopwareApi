...
class KueblerContext
{
    ...
    {
		Log::channel('stock')->notice('kuebler Token valid? = ' . (time() - (int)(config('shopware-api.access_token_expires_at.kuebler'))));

		if (time() - (int)(config('shopware-api.access_token_expires_at.kuebler')) > (8 * 60)) {
			Log::channel('stock')->notice('Getting new token.');
			Artisan::call('config:clear');
			$grantType = new ClientCredentialsGrantType(config('shopware-api.access_key.kuebler'),config('shopware-api.secret_access_key.kuebler'));
			...
			config([
				'shopware-api.access_token.kuebler' => $accessToken,
				'shopware-api.access_token_expires_at.kuebler' => strtotime('now'),
			]);
			...
			$accessToken = config('shopware-api.access_token.kuebler');
		}
		return $accessToken;
    }
}
