...
class UvexContext
{
    ...
    {
		Log::channel('stock')->notice('uvex Token valid? = ' . (time() - (int)(config('shopware-api.access_token_expires_at.uvex'))));
		...
		if (time() - (int)(config('shopware-api.access_token_expires_at.uvex')) > (8 * 60)) {
			...
			config([
				'shopware-api.access_token.uvex' => $accessToken,
				'shopware-api.access_token_expires_at.uvex' => strtotime('now'),
			]);
