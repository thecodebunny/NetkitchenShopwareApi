...
class CustomerImportContext
{
    ...
    {
		Log::channel('customers')->notice('Token valid? = ' . (time() - (int)(config('shopware-api.access_token_expires_at.customer_import'))));

		if (time() - (int)(config('shopware-api.access_token_expires_at.customer_import')) > (8 * 60)) {
			Log::channel('customers')->notice('Getting new token.');
			//Artisan::call('config:clear');
			$grantType = new ClientCredentialsGrantType(config('shopware-api.access_key.customer_import'),config('shopware-api.secret_access_key.customer_import'));
			...
			config([
				'shopware-api.access_token.customer_import' => $accessToken,
				'shopware-api.access_token_expires_at.customer_import' => strtotime('now'),
			]);
			...
			$accessToken = config('shopware-api.access_token.customer_import');
		}
		return $accessToken;
    }
}
