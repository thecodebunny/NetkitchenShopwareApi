...
class StockUpdateContext
{
    ...
    {
		Log::channel('stock')->notice('stock_update Token valid? = ' . (time() - (int)(config('shopware-api.access_token_expires_at.stock_update'))));

		if (time() - (int)(config('shopware-api.access_token_expires_at.stock_update')) > (8 * 60)) {
			Log::channel('stock')->notice('Getting new token.');
			//Artisan::call('config:clear');
			$grantType = new ClientCredentialsGrantType(config('shopware-api.access_key.stock_update'),config('shopware-api.secret_access_key.stock_update'));
			...
			config([
				'shopware-api.access_token.stock_update' => $accessToken,
				'shopware-api.access_token_expires_at.stock_update' => strtotime('now'),
			]);
			...
			$accessToken = config('shopware-api.access_token.stock_update');
		}
		return $accessToken;
    }
}
